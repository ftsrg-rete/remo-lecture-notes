...
In the interface package create command interface:
  ...
  - Create the "configure" operation in the "ConfigurationInterface"
    ...
    - Double click on the operation **in the diagram** and set the parameters and the return type
      - in parameter deafaultTemperature : Temperature
...
In the Interfaces BDD create interface blocks for the system context diagram:
  ...
  ![alt text](figs/ea-tooling-structure/image-25.png)
    - Battery IF
      - BAT_V : ChargeFlowElement
      - GND : ChargeFlowElement
    - Vehicle Communication IF
      - CAN_P : ChargeFlowElement
      - CAN_N : ChargeFlowElement
  ...
  - Draw a "Generalsation" relationship from the "Mechanical Connection IF" to the "EntropyFlowElement"
...
In the Interfaces BDD create association blocks for the battery wire:
  ...
  - In the "Interaction Points" tab of the "Features" window click on "All" button to visualize the ports
...
Create the composition structure of the system context:
  ...
  - Drag the Power value type to the Vehicle Battery block and drop as "Propery"
...
Create IBD for the System Context:
  ...
  - Drag the "Battery IF", "Vehicle Communication IF", "Mechanical Connection IF" to the propeties and drop as ports. After that, name the ports. 
